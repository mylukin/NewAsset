        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="设备类型" prop="facilityType">
              <el-select v-model="form.facilityType" placeholder="选择设备类型" style="width: 100%">
          ...
          <el-col :span="12">
            <el-form-item label="维护周期">
              <el-input-number v-model="form.maintenanceCycle" :min="0" placeholder="请输入维护周期" style="width: 100%" />
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="购置价格">
              <el-input-number v-model="form.purchasePrice" :precision="2" :step="1000" :min="0" placeholder="请输入购置价格" style="width: 100%" />
...
<script>
import { getFacility, addFacility, updateFacility } from '@/api/asset/facility'

export default {
  name: 'FacilityAssetForm',
  props: {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        facilityType: [{ required: true, message: '请选择设备类型', trigger: 'change' }]
      },
      submitting: false
    }
  },
  ...
  methods: {
    getAssetDetail() {
      getFacility(this.assetId).then(response => {
    ...
    onSubmit() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          ...
          const submitMethod = this.isEdit ? updateFacility : addFacility
...
<style scoped>
.facility-asset-form { padding: 20px; }
