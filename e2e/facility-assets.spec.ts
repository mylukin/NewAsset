...
test.describe('Facility Assets Page', () => {
  ...
  test.describe('Page Load', () => {
    test('should load facility assets page successfully', async ({ page }) => {
      await expect(page).toHaveURL(/.*\/asset\/facility/);
      await expect(page.locator('body')).toBeVisible();
    });

    test('should display facility assets container', async ({ page }) => {
      await expect(page.locator('.facility-container')).toBeVisible();
    });

    test('should display page heading', async ({ page }) => {
      await expect(page.locator('h2:has-text("Facility Assets")')).toBeVisible();
    });

    test('should display module description', async ({ page }) => {
      await expect(page.locator('p:has-text("Facility asset management module")')).toBeVisible();
    });
  });

  test.describe('Navigation', () => {
    test('should navigate from dashboard to facility assets', async ({ page }) => {
      await page.goto('/');
      await page.waitForLoadState('networkidle');
    ...
    test('should navigate via sidebar menu', async ({ page }) => {
      ...
      // Click Facility Assets menu item
      await page.click('.el-menu-item:has-text("Facility Assets")');
      await page.waitForLoadState('networkidle');
  ...
  test.describe('Layout Integration', () => {
    ...
    test('should have correct sidebar menu item active', async ({ page }) => {
      // The Facility Assets menu item should be active
      const menuItem = page.locator('.el-menu-item[index="/asset/facility"]');
  ...
  test.describe('Content Display', () => {
    test('should have proper styling', async ({ page }) => {
      const container = page.locator('.facility-container');
