...
test.describe('Parking Assets Page', () => {
  ...
  test.describe('Page Load', () => {
    test('should load parking assets page successfully', async ({ page }) => {
      await expect(page).toHaveURL(/.*\/asset\/parking/);
      await expect(page.locator('body')).toBeVisible();
    });

    test('should display parking assets container', async ({ page }) => {
      await expect(page.locator('.parking-container')).toBeVisible();
    });

    test('should display page heading', async ({ page }) => {
      await expect(page.locator('h2:has-text("Parking Assets")')).toBeVisible();
    });

    test('should display module description', async ({ page }) => {
      await expect(page.locator('p:has-text("Parking asset management module")')).toBeVisible();
    });
  });

  test.describe('Navigation', () => {
    test('should navigate from dashboard to parking assets', async ({ page }) => {
      await page.goto('/');
      await page.waitForLoadState('networkidle');
    ...
    test('should navigate via sidebar menu', async ({ page }) => {
      ...
      // Click Parking Assets menu item
      await page.click('.el-menu-item:has-text("Parking Assets")');
      await page.waitForLoadState('networkidle');
  ...
  test.describe('Layout Integration', () => {
    ...
    test('should have correct sidebar menu item active', async ({ page }) => {
      // The Parking Assets menu item should be active
      const menuItem = page.locator('.el-menu-item[index="/asset/parking"]');
  ...
  test.describe('Content Display', () => {
    test('should have proper styling', async ({ page }) => {
      const container = page.locator('.parking-container');
